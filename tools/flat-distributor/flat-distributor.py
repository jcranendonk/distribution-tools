...
def transfer(
    ...
    if supply_code != 0:
        sys.exit(
            "Exiting, failed to read the current supply account address. Try checking the output of 'solana address'."
        )

    print(
        f"{bcolors.DANGER}WARNING: MAKE SURE YOU ARE USING THE CORRECT WALLET/SUPPLY/ TO DISTRIBUTE.\nYOUR CURRENT WALLET ADDRESS IS: {current_supply.decode('utf-8')}{bcolors.ENDC}"
    )
    print(
