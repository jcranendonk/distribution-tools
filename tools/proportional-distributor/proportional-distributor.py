...
def continue_airdrop_prompt(interactive, separator):
    ...
    interactive_msg = (
        f"{bcolors.BOLD}You will be prompted to confirm each transaction.{bcolors.ENDC}"
    )
...
class TransferCmd:
    ...
    def to_list(self):
        ...
        obj = [
            ...
            self.instruction,
            self.mint_address,
            f"{self.drop_amount:.{self.decimals}f}",
            self.recipient,
            "--url",
            self.url,
        ]
    ...
    while True:
        ...
        if code == 0:
            ...
            if RPC_ERROR in err_msg:
                print("-32005 RPC Error, waiting 5... ", end="", flush=True)
            ...
            if UNCONFIRMED in err_msg or TOO_MANY_REQUESTS in err_msg:
                print(f"{bcolors.DANGER}UNCONFIRMED{bcolors.ENDC}", flush=True)
...
def get_balance(addr, addr_type, mint, url):
    if addr_type == "token":
        ...
        if ok:
            cmd = ["spl-token", "balance", "--address", assoc_addr, "--url", url]
            code, output, _ = run(cmd)
            if code == 0 and output.decode("utf-8") != "":
                return True, float(output.decode("utf-8"))
            else:
            ...
            if ok:
                try:
                    ...
                    if diff >= 0:
                        ...
                        startc = "\033[91m"
                    output_line += (
                        f"{actual:.{TOKEN_DECIMALS}f},{diff:.{TOKEN_DECIMALS}f}"
                    )
                except TypeError:
...
def transfer(
    input_path, interactive, drop_amount, fund_recipient, allow_unfunded_recipient
):
    ...
    if supply_code != 0:
        sys.exit(
            "Exiting, failed to read the current wallet account address. Try checking the output of 'solana address'."
        )

    print(
        f"{bcolors.DANGER}WARNING: MAKE SURE YOU ARE USING THE CORRECT WALLET TO DISTRIBUTE.\
...
parser_b = subparsers.add_parser(
    "check-before",
    help="Checker before a distribution, generates a CSV file with a snapshot of current balances of all accounts, and the expected balance after the distribution.",
)
...
parser_a = subparsers.add_parser(
    "check-after",
    help="Run the checker after a distribution, to check if all recipients received the expected amount of tokens.",
)
...
parser_t = subparsers.add_parser(
    "transfer",
    help="Distribute a proportional amount of tokens to all given addresses.",
)
